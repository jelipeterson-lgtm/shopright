...
| Smart Circle template column shift | June 12, 2026 | Smart Circle removed col 12 "Less than 4 reps present? (Pass/Fail)" from their 6/12/26 template. Every column from old col 13 onward shifted left by 1 in new template (39 data cols, Visit Recap now at col 39/AM instead of col 40/AN). Updated excel.py to match. Requires Eli to upload new template to Dropbox as "Upload new version" to preserve existing shared link URL. |
| Route stop completion bug fix | June 18, 2026 | Stop completion logic used a count match (visits.length >= planned vendor count) instead of name matching. Result: adding and completing an unplanned vendor (e.g. CKE) satisfied the count for a different planned vendor (e.g. Water), prematurely moving the store to Completed. Fixed to match each planned vendor by program name to a Complete visit. Also fixed Open vendor rows in the Completed section — they were disabled (not tappable) when no visit record existed. Now they behave like the Upcoming section: tap creates the visit and navigates directly to the assessment. |
| Invoice download crash fixed | August 26, 2026 | `generate_invoice_endpoint` passed pydantic `MileageEntry` objects straight to `generate_invoice()`, which calls `.get()` on each entry expecting a dict — crashed with a 500 ("Download failed") whenever a shopper entered mileage. Fixed to convert to dicts first, matching the pattern `send_invoice` already used correctly. Also fixed `end_day` default (1→31) and the within-month vs. cross-month date-range logic in both invoice endpoints to match the frontend's Settings page behavior. |
| Render OOM recurrence investigated | August 26, 2026 | Two new OOM incidents (Aug 20 hard kill, Aug 21 controlled self-restart) months after the June 11 "resolved" status. Growth is bursty (100+ MB within 12–17 min) during concurrent shopper autosave traffic, not a steady leak. Ruled out per-call/per-thread Supabase client creation (it's a shared singleton). Found postgrest-py hardcodes `http2=True` with no toggle, and the Aug 21 traceback shows an HTTP/2 connection-reset error in that exact client — an open lead, not yet acted on. Confirmed with Eli that Render has not been upgraded from the Free tier. |
| Thread-count instrumentation added | August 26, 2026 | Added `threading.active_count()` to the `[keep-alive]` RSS log line and a new middleware that logs `[threads] N active — METHOD /path` on any request above 15 concurrent threads. Deployed to test the leading hypothesis (thread-pool growth under bursty concurrent load) against real traffic before making any concurrency-limiting change. |
...
| Render OOM (recurring) | Reopened Aug 26, 2026 | Two new incidents Aug 20–21 despite June 11 "resolved" status. Thread-count instrumentation deployed; awaiting next high-traffic burst to confirm cause. Render confirmed still on Free tier. |
| Batch-create visit_time NOT NULL bug | Open | `POST /visits/batch` intermittently 500s with a `visit_time` NOT NULL constraint violation — contradicts the documented design (batch-created visits should have null visit_time, set later on first open). Needs a Supabase migration to make the column nullable; blocked on DB access as of Aug 26, 2026. |
...
| June 12, 2026 | Template column shift | Smart Circle's new 6/12/26 shop file template removed old col 12 "Less than 4 reps present? (Pass/Fail)". Compared both templates column-by-column to discover structural change (user reported only colors/header text changed). Updated excel.py: removed Pass/Fail write for old col 12; shifted rep_count col 13→12; shifted all 26 eval field writes cols 14–40 → 13–39; fixed Fail/N/A branches (col 40→39, range 10–40 → 10–39). Eli replaced Dropbox template file (overwrite, same URL). Render redeploy triggered — template cache cleared on restart. | Kelsey real-world validation pending |
| June 18, 2026 | Route completion bug fix | Diagnosed Rebecca Taggart bug: Becca added unplanned CKE vendor to Costco #1027, completed CKE, and the store moved to Completed even though the planned Water vendor was never done. Root cause: completion check used count (storeVs.length >= vendorCount) instead of name match, so CKE's complete visit satisfied Water's slot. Fixed: each planned vendor must have a matching Complete visit by program name. Second fix: Open vendors in the Completed section were disabled (no visit record = untappable). Fixed to use the same createVisit+navigate flow as the Upcoming section. | Vercel auto-deploy triggered |
| August 26, 2026 | Bug fixes + OOM investigation | Fixed invoice download crash: `generate_invoice_endpoint` was passing pydantic `MileageEntry` objects to `generate_invoice()` instead of dicts, causing a 500 whenever mileage was entered. Also fixed `end_day` default and date-range logic in both invoice endpoints. Investigated Render OOM recurrence (Aug 20 hard kill, Aug 21 self-restart): traced to bursty RSS growth (100+ MB in 12–17 min) during concurrent shopper autosave traffic, not a steady leak. Ruled out per-call Supabase client creation as the cause. Added `threading.active_count()` instrumentation to the keep-alive log and a burst-detection middleware to test whether anyio's sync-call thread pool is driving it. Found an open lead (postgrest-py's hardcoded HTTP/2 client, tied to an Aug 21 connection-reset traceback) but left it unfixed pending confirmation. Found a separate, unrelated bug: `POST /visits/batch` violates a `visit_time` NOT NULL constraint — not yet fixed, blocked on Supabase DB access. Confirmed with Eli that Render has not been upgraded from the Free tier. | Awaiting next high-traffic burst — check `[threads]` and `[keep-alive]` Render logs for correlation with RSS |
