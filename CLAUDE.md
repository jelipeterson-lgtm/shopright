...
|--------|------|-------|
| id | serial (PK) | |
| retailer_name | text | Costco, Kroger - Fred Meyer, Target |
| store_number | text | |
| account | text | |
| program | text | e.g., RTL-ATT-EDM |
...
| GET | /stores/programs | Yes | Get programs for a store |
...
|------|---------|------------|
| Book1.xlsx | Store directory — retailer, store #, program, address | Eli, when Smart Circle changes stores |
...
Backend downloads these on demand and caches in memory.
